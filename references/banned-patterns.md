...
| Em-dash overuse | "Your export is ready — download it now — before the link expires." | "Your export is ready. Download it before the link expires." |
...
In write mode, check a draft against every row before it ships. In strip mode, name the specific row(s) that applied to the original copy in the "Removed" line of the output - "Removed: promotional adjective, em-dash overuse" is more useful to the user than "Removed: AI tells".
